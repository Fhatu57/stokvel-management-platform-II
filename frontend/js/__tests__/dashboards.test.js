import { initMemberDashboard, initTreasurerDashboard, initMyGroups } from '../dashboards.js';

describe('initMemberDashboard - DOM setup', () => {
  beforeEach(() => {
    localStorage.clear();
    localStorage.setItem('stokvel_user', JSON.stringify({ name: 'Lucky', role: 'member' }));
    document.body.innerHTML = `
      ...
      <section id="payout-info"></section>
...
describe('initTreasurerDashboard - DOM setup', () => {
  beforeEach(() => {
    localStorage.clear();
    localStorage.setItem('stokvel_user', JSON.stringify({ name: 'Nomsa', role: 'treasurer' }));
    document.body.innerHTML = `
      <section id="groups-container"></section>
      <p id="stat-group-count"></p>
      <p id="stat-monthly-total"></p>
...
describe('initMyGroups - DOM setup', () => {
  beforeEach(() => {
    ...
    localStorage.setItem('stokvel_user', JSON.stringify({ name: 'Lucky', role: 'member' }));
    document.body.innerHTML = `
      <section id="my-groups-container"></section>
      <section id="groups-container"></section>
    `;
  });
...
describe('dashboard stats logic', () => {
  test('GIVEN groups array, WHEN monthly total calculated, THEN sum is correct', () => {
    ...
    const total = groups.reduce((s, g) => s + Number(g.contribution_amount || 0), 0);
  ...
  test('GIVEN contributions, WHEN pending filtered, THEN count is correct', () => {
    ...
    expect(contributions.filter(c => c.status === 'pending').length).toBe(2);
  });
...
describe('group status badge logic', () => {
  function getStatusClass(status) {
    return status === 'active' ? 'badge-success'
         : status === 'inactive' ? 'badge-warning' : 'badge-error';
  }
...
describe('monthly contribution calculation', () => {
  ...
  test('GIVEN no paid_at, WHEN total calculated, THEN returns 0', () => {
    expect(calcMonthlyTotal([{ amount: 500, paid_at: null }])).toBe(0);
  });

  test('GIVEN current month contribution, WHEN total calculated, THEN includes it', () => {
    const now = new Date().toISOString();
    expect(calcMonthlyTotal([{ amount: 500, paid_at: now }])).toBe(500);
  });

  test('GIVEN old contribution, WHEN total calculated, THEN excludes it', () => {
    expect(calcMonthlyTotal([{ amount: 500, paid_at: '2020-01-01T00:00:00Z' }])).toBe(0);
  });
});